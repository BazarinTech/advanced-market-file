...
if(isset($_POST['deposit'])){
            ...
            $time = time();
            $sql = "SELECT * FROM earnings WHERE email='".$uname."'";
            ...
            $status = 'Success';
            $sql = "UPDATE earnings SET balance=$balance, deposit=$deposit WHERE email='".$uname."'";
            $res = mysqli_query($conn, $sql);
            $sql2 = "INSERT INTO transaction (`email`, `phone`, `amount`, `type`, `status`) VALUES('".$uname."', '".$phone."', '".$amount."', 'Deposits', '".$status."')";
   ...
   <div id="sidebar" class="w3-cell w3-padding w3-block w3-quarter w3-blue" style="width: fit-content; display: flex; flex-direction: column; align-items: start; overflow-x: hidden;">
          ...
          <div style="padding: 0; margin: 0; display: flex; flex-direction: column; align-items: center; justify-content: center;" class="w3-cell w3-container w3-threequarter">
            ...
            <div style="display: flex; flex-direction:column; align-items:center; justify-content:center">
                <form style="display: flex; flex-direction:column; align-items:start; justify-content:center;" class="w3-card w3-white w3-margin w3-padding w3-round" action="deposits.php" method="post">
                    <h3 style="font-weight: bold;" class="w3-large w3-text-blue">Enter amount you wish to deposit</h3>
                    <label for="inpt-amount" class="w3-large">Amount</label>
                    <input style="outline: none;" type="number" class="w3-large w3-margin-bottom w3-input" id="inpt-amount" name="amount" placeholder="Enter Amount(Kes)" required>
                    <label for="inpt-phone" class="w3-large">Phone Number </label>
                    <input style="outline: none;" class="w3-large w3-input w3-margin-bottom" type="tel" name="phone" id="inpt" placeholder="07xxxxxxxxx" required>
                    <label for="inpt-amount" class="w3-large">Email</label>
                    <input style="outline: none;" type="email" class="w3-large w3-margin-bottom w3-input" id="inpt-amount" name="email" placeholder="Enter Email" required>
                    <label for="inpt-amount" class="w3-large">TransactionID</label>
                    <input style="outline: none;" type="text" class="w3-large w3-margin-bottom w3-input" id="inpt-amount" name="transactionID" placeholder="Enter TransactionID" required>
                    <input style="width: 50%;" type="submit" value="Deposit" class="w3-btn w3-teal w3-hover-amber" name="deposit">
                </form>
                <h2 style="font-weight: bold;" class="w3-centered w3-xxlarge w3-text-teal">All Deposits</h2>
            <div style="width: 90vw; display: grid; place-items: center" class="w3-card w3-white w3-margin w3-padding w3-round">
             <table style="margin: auto;" id='investTable' class="w3-table-all w3-responsive w3-margin w3-centered">
                ...
                <?php 
                    $sql = "SELECT * FROM transaction WHERE type='Deposits'";
                    $res = mysqli_query($conn, $sql);
                    $val = 1;
                    foreach($res as $row):
                ?>
