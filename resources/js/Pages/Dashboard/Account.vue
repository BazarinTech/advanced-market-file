...
const menuItems = [
    ...
    { route: 'packages.table', icon: 'table-list', label: 'Investment Plans', desc: 'Compare all available plans' },
] as const;
...
<template>
    ...
    <AppLayout show-bottom-nav>
        ...
        <div class="w-full px-4 mt-4">
            <div class="w-full rounded-xl p-5 grid grid-cols-3 text-center bg-card border border-border">
                <div class="flex flex-col gap-1.5 items-center">
                    <p class="text-muted-foreground text-[9px] tracking-widest uppercase">Balance</p>
                    <p class="text-white text-sm font-medium">Kes {{ money(earnings.balance) }}</p>
                </div>
                <div class="flex flex-col gap-1.5 items-center border-l border-r border-border">
                    ...
                    <p class="text-white text-sm font-medium">Kes {{ money(earnings.deposit) }}</p>
                </div>
                <div class="flex flex-col gap-1.5 items-center">
        ...
        <div class="w-full px-4 mt-4 mb-28 flex flex-col gap-2">
            <Link
                ...
                class="flex items-center justify-between px-4 py-3.5 rounded-xl no-underline transition-colors bg-card border border-border"
            >
                <span class="flex items-center gap-3">
                    ...
                    <span>
                        <span class="block text-white text-sm tracking-wide font-light">{{ item.label }}</span>
            ...
            <form class="mt-2" @submit.prevent="logout">
                <button
                    ...
                    class="w-full flex items-center justify-between px-4 py-3.5 rounded-xl transition-colors text-left disabled:opacity-70"
                ...
                >
                    ...
                    <Icon name="angle-right" class="text-sm" style="color: #3a1a1a" />
                </button>
            </form>
        </div>
    </AppLayout>
